...
| **`xcodebuild` exit code 69** | Often **Xcode license** not accepted or **first launch** not finished. Run `sudo xcodebuild -license accept`, open **Xcode** once and complete prompts, then `sudo xcode-select -s /Applications/Xcode.app/Contents/Developer` (use **Xcode-beta.app** if that is what you installed). |
| **WDA failure — need build log** | Set `session.show_xcode_log: true` in YAML (default in `config_template.yaml`); framework passes it to Appium as `showXcodeLog`. Watch the **Appium** terminal for the real `xcodebuild` error. |
| **`iOS 26.x is not installed`** / no iPhone destination | Xcode is missing the **device OS platform** for your iPhone. **Xcode → Settings → Platforms** (or **Components**) → download the **iOS** version the log names (e.g. **26.4**). Update `device.platform_version` in YAML to match **Settings → General → About** on the phone. |
| **Deployment target doesn’t match device** | Log: *iOS X doesn’t match WebDriverAgentRunner’s iOS Y deployment target*. Set `device.platform_version` in YAML to the **exact** OS on the phone (e.g. **26.3.1**). |
| **No profiles for WebDriverAgentRunner** / **allowProvisioningUpdates** | Set `session.allow_provisioning_device_registration: true` in YAML (default in `config_template.yaml`) so Appium passes **-allowProvisioningUpdates** to `xcodebuild`. |
| **No signing certificate "iOS Development"** | **Xcode → Settings → Accounts** → your Apple ID → **Manage Certificates…** → **+** → **Apple Development**. Ensure `xcode.signing_id` is **`Apple Development`** and `xcode.org_id` is your **Team ID**. |
| **`com.facebook.WebDriverAgentRunner.xctrunner` cannot be registered** | Free **Personal Team** can't claim Facebook's id. Set `app.updated_wda_bundle_id` to something unique like `com.<yourname>.WebDriverAgent` (Appium appends `.xctrunner` to the runner target). |
| **`codesign wants to access key … in your keychain` keeps stopping the build** | macOS prompts before letting `codesign` use the Apple Development private key. Click **Always Allow** so it stops blocking, or in **Keychain Access** open the key → **Get Info → Access Control → Always allow** (or add `/usr/bin/codesign`). |
| **WDA built but `/status` times out / `socket hang up` to `127.0.0.1:8100`** | First install on a **Personal Team** needs to be **Trusted** on the phone: **Settings → General → VPN & Device Management → Developer App → trust your Apple ID**. Then re-run. Also bump `session.wda_launch_timeout` (e.g. 240000) and `session.wda_startup_retries` (e.g. 2) for slow first installs. |
